...
const QUICK_REPLIES = [
  ...
  ["Ja, ik huur", "Nee, koopwoning", "Ik woon bij familie"],
  ["Minder dan €2000", "€2000 - €3000", "Meer dan €3000"],
];
...
export default function AssistentPage() {
  ...
  const currentQuickReplies = QUICK_REPLIES[Math.min(responseIndex, QUICK_REPLIES.length - 1)];
  ...
  return (
    <div className="flex flex-col h-screen pb-20 md:pb-0">
      ...
      {/* Quick replies */}
      {!isTyping && responseIndex < MOCK_RESPONSES.length && (
          ...
          <div className="flex gap-2 overflow-x-auto pb-2 max-w-3xl mx-auto">
            {currentQuickReplies.map((reply) => (
              <button
                ...
                className="flex-shrink-0 px-4 py-2 bg-[var(--muted)] rounded-full text-sm font-medium hover:bg-[var(--border)] transition-colors cursor-pointer"
              >
                {reply}
              </button>
            ))}
      ...
      {/* Input area */}
      <div className="flex-shrink-0 px-4 pb-4 pt-2 border-t border-[var(--border)] bg-[var(--background)]">
        <div className="flex gap-2 items-end max-w-3xl mx-auto">
          <div className="flex-1 relative">
            <textarea
              ...
              className="w-full px-4 py-3 pr-12 rounded-2xl border border-[var(--border)] bg-[var(--muted)] resize-none focus:outline-none focus:ring-2 focus:ring-[var(--primary)] text-sm"
              rows={1}
          ...
          <button
            ...
            className="w-12 h-12 rounded-full bg-[var(--primary)] text-white flex items-center justify-center disabled:opacity-50 disabled:cursor-not-allowed transition-opacity cursor-pointer"
          >
